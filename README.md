...
```
速度与激情8 (2017) [2160P HEVC REMUX] {tmdb-337339}          ← 4K 主版本
速度与激情8 (2017) [2160P HEVC REMUX] {tmdb-337339} - PROPER ← 修正版
速度与激情8 (2017) [1080P 蓝光]      {tmdb-337339}          ← 1080P 靠画质区分
速度与激情8 (2017) [2160P HEVC REMUX] {tmdb-337339} - v2     ← 真·同名副本
```

**顺便修掉的三个毛病**

1. **文件夹画质张冠李戴** —— `速度与激情8 ...- 1080p BluRay (2)` 这个文件夹
   明明是 1080P，却被写成了 `[2160P HEVC REMUX]`。原因是从条目那里继承了画质。
   现在**文件夹优先看自己的名字**，名字里写着 1080p 就用 1080P。
2. **英文原名丢中文名** —— `The.Fast.and.the.Furious.2001.BluRay...CMCT.mkv`
   直接挂在分享根目录下，父目录解析不出身份，结果中文名和 TMDB 全丢了，
   改成半截英文名。现在建了张「全量身份表」，靠**年份唯一**或**片名归一化**
   反向认领中文名 → `速度与激情 1 (2001) [1080P H.264 蓝光] {tmdb-9799}.mkv`。
3. **英文点号没拆开** —— `The.Fast.and.the.Furious` 里的点没被当分隔符处理，
   导致和中文名匹配不上。现在纯英文名会把点号转成空格。
   （中文名的全角点不受影响，`速度与激情：特别行动` 照旧。）

**实测（你的账号 U_0820，速度与激情合集 26 项计划）**
...
| 真实改名 + 还原 | ✅ 改完精确还原，云盘无残留 |

## v1.1.2 (2026-09-11)

**用真实账号实测后修的三个问题**（这次是真跑通了，不是纸上谈兵）：

1. **令牌填了登不上** —— 你给的 access_token 已过期（它只活 2 小时），
   而旧代码不会拿 refresh_token 去续期，直接拿过期 token 去验就报错。
   现在 `user_info` 遇到 401 会**自动用 refresh_token 换新令牌重试**。
2. **空壳文件夹被乱改** —— 速度与激情合集里 Ⅲ/Ⅳ/Ⅴ 三部，文件夹里
   **只剩截图和字幕，视频早就没了**。旧逻辑照样改名，结果生成
   "速度与激情 3"（没年份、没 TMDB）这种残缺名字。
   现在**只改确实装着视频的文件夹**，空壳目录一律跳过。
3. **多版本文件夹撞名** —— 同一部片有 1080P、4K 两个文件夹，
   规范化后名字一模一样。现在自动加 `-2`、`-3` 后缀区分。

**实测结论（用你的账号 U_0820 真跑）**：

| 验证项 | 结果 |
| ---- | ---- |
| refresh_token 续期 | ✅ 换出新 token，`expires_in: 7200` |
| 读取账号信息 | ✅ `U_0820` / `+86 136***820` |
| 列网盘目录 | ✅ 读到 11 个根目录 |
| **访问令牌=分享页令牌** | ✅ 分享里的文件 `fullParentIds` 指向你的「电影」目录 |
| **文件重命名** | ✅ 英文名 → `爱的秘密 (1997) [1080P H.264 WEBRip] {tmdb-12723}.mkv` |
| **文件夹重命名** | ✅ `A-爱的秘密(1997)` → `爱的秘密 (1997) [1080P H.264 WEBRip] {tmdb-12723}` |
| 根目录保护 | ✅ 「XX 合集」不会被改 |
| 测试后还原 | ✅ 已全部还原，网盘无残留改动 |
