...
```
📁 A-示例电影(1997)  →  示例电影 (1997) [1080P H.264 WEBRip] {tmdb-12723}
🎞 示例电影.mkv       →  示例电影 (1997) [1080P H.264 WEBRip] {tmdb-12723}.mkv
```
...
**同一部电影多个版本，重命名不再撞名**（你提的需求）。

一个分享里同一部片常有 1080P、4K、PROPER、国配等多个版本，改完名字全一样，
系统只能给它们随便加 `(2)` `(3)`，转存进 Emby 一看就懵。现在改成：

**怎么区分（按优先级挑，第一个能用的就用）**

| 次序 | 用什么 | 例子 |
| ---- | ---- | ---- |
| ① | 画质不同 | 1080P 版自带 `[1080P H.264 蓝光]`，直接就和 4K 版分开了 |
| ② | 版本词 | `PROPER`、`EXTENDED`、`导演剪辑`、`未删减`、`IMAX` |
| ③ | 音轨/字幕 | `国配双语`、`3D` |
| ④ | 发布组 | `FGT`、`CMCT死亡骑士`、`HomeWEB` |
| ⑤ | 分辨率/编码 | `2160P`、`1080P`、`HEVC` |
| ⑥ | 两两组合 | `PROPER · FGT` |
| ⑦ | 纯序号兜底 | `- v2`（实在一点线索都抽不出来时才用） |

**只有一个版本时，一个字都不加** —— 不画蛇添足。

**实际效果**（示例电影四个版本）：

```
示例电影 (2017) [2160P HEVC REMUX] {tmdb-337339}          ← 4K 主版本
示例电影 (2017) [2160P HEVC REMUX] {tmdb-337339} - PROPER ← 修正版
示例电影 (2017) [1080P 蓝光]      {tmdb-337339}          ← 1080P 靠画质区分
示例电影 (2017) [2160P HEVC REMUX] {tmdb-337339} - v2     ← 真·同名副本
```

**顺便修掉的三个毛病**

1. **文件夹画质张冠李戴** —— `示例电影 ...- 1080p BluRay (2)` 这个文件夹
   明明是 1080P，却被写成了 `[2160P HEVC REMUX]`。原因是从条目那里继承了画质。
   现在**文件夹优先看自己的名字**，名字里写着 1080p 就用 1080P。
2. **英文原名丢中文名** —— `Some.Movie.2001.BluRay.1080p.x264-GROUP.mkv`
   直接挂在分享根目录下，父目录解析不出身份，结果中文名和 TMDB 全丢了，
   改成半截英文名。现在建了张「全量身份表」，靠**年份唯一**或**片名归一化**
   反向认领中文名 → `示例电影 (2001) [1080P H.264 蓝光] {tmdb-9799}.mkv`。
3. **英文点号没拆开** —— `Some.Movie.Name` 里的点没被当分隔符处理，
   导致和中文名匹配不上。现在纯英文名会把点号转成空格。
   （中文名的全角点不受影响，`示例电影：副标题` 照旧。）
...
| 文件夹重名 | ✅ 15 项，0 重名 |
| 文件重名 | ✅ 11 项，0 重名 |
| 4K / PROPER / 1080P 三版本区分 | ✅ 各带信息量后缀 |
| 真·同名副本 | ✅ 落到 `- v2` |
| 文件夹画质正确性 | ✅ `1080p BluRay` → `[1080P 蓝光]` |
| 真实改名 + 还原 | ✅ 改完精确还原，无残留 |

## v1.1.2 (2026-09-11)

**修掉三个会让功能完全不可用/出错误名的问题**：

1. **令牌过期后无法续期** —— access_token 只活 2 小时，旧代码不会拿
   refresh_token 去换新的，直接拿过期 token 去验就报错。
   现在 `user_info` 遇到 401 会**自动用 refresh_token 换新令牌重试**。
2. **空壳文件夹被乱改** —— 合集里有的目录**只剩截图和字幕，视频早就没了**。
   旧逻辑照样改名，结果生成"示例电影 3"（没年份、没 TMDB）这种残缺名字。
   现在**只改确实装着视频的文件夹**，空壳目录一律跳过。
3. **多版本文件夹撞名** —— 同一部片有 1080P、4K 两个文件夹，
   规范化后名字一模一样。现在自动加 `-2`、`-3` 后缀区分。

**实测结论**：

| 验证项 | 结果 |
| ---- | ---- |
| refresh_token 续期 | ✅ 换出新 token，`expires_in: 7200` |
| 读取账号信息 | ✅ 正常返回昵称与手机号 |
| 列网盘目录 | ✅ 能列出根目录全部文件夹 |
| **访问令牌=分享页令牌** | ✅ 分享里的文件 `fullParentIds` 指向网盘自己的目录 |
| **文件重命名** | ✅ 英文名 → `示例电影 (1997) [1080P H.264 WEBRip] {tmdb-12723}.mkv` |
| **文件夹重命名** | ✅ `A-示例电影(1997)` → `示例电影 (1997) [1080P H.264 WEBRip] {tmdb-12723}` |
| 根目录保护 | ✅ 「XX 合集」不会被改 |
| 测试后还原 | ✅ 改名可精确还原，无残留改动 |
...
- 新增 `enrich_items_with_tmdb()`：合集里没有 `{tmdb-id}` 的条目，按「片名 + 年份」自动搜 TMDB
  补 id 与中文名 → 封面从「剧照拼图」升级为**官方海报**（如某演员合集 42 部命中 33/35）
...
- `parse_share_title(..., is_share_title=True)`：分享总标题不再套用「取方括号中文片名」规则，
  修复 `【某演员】电影合集【42部】` 被错切成「某演员」、丢失「合集」语义
- 新增 `is_junk_title()` + 收紧 `MEDIA_EXTS`：过滤蓝光原盘结构名（`BDMV`/`STREAM`/`AUXDATA`/
  ...
  导致片单污染、年份跨度异常（某合集年份跨度被算错的 bug）
- 片名清洗增强：去演员合集的字母分类前缀（`M-示例电影` → `示例电影`，仅短横线后接中文时）、
  去方括号版本说明（`[60帧率版本][高码版]`）、去无括号发布组水印（`高清影视之家发布`）
- 合集封面底栏文字：过长时优先在分隔符处断开，避免「XX合集」后缀被截成「XX合…」

## v1.0.6 (2026-09-10)

- 修复「上传者把多部片错误塞进同一个 `片名(年份){tmdb}` 文件夹」导致所有视频都继承错误身份、整组合集退化为英文片名的问题（如某系列合集：4 部片全被塞进同一个 "XX (1999) {tmdb-603}" 文件夹）
...
- 合集片单"同系列去重"：标题去掉尾部序号后相同视为同一部，避免把"示例电影 (1999) {tmdb-603}"和"示例电影1 1080P"重复计入
...
- 文件大小字段兼容 `fileSize`；子文件继承最近一层目录的 `{tmdb-id}`、中文片名（如 `[示例电影2].2.Example.Movie...` 目录名 + 英文文件名 → 取中文片名 + TMDB ID）
- 合集片单去重：同一年份跨语言/多版本自动合并，避免「示例电影2 / 示例电影2 - 4K REMUX 4K」重复
- 合集封面 4 级兜底，保证一定能出图：
...
- 支持分享合集（演员合集 / 系列三部曲 / 系列 1-4 等）：自动递归列出目录，识别每部片名/年份/TMDB ID/画质，生成带片单的合集帖
